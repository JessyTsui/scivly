...
def test_authenticated_interests_request_is_workspace_scoped(client: TestClient, auth_headers: Callable[..., dict[str, str]]) -> None:
    response = client.get(
        ...
        headers=auth_headers(workspace_id="00000000-0000-0000-0000-000000000201"),
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["total"] == 2
    assert {item["workspace_id"] for item in payload["items"]} == {"00000000-0000-0000-0000-000000000201"}
...
def test_papers_and_scores_read_from_database(
    ...
    score_response = client.get(
        f"/papers/{first_paper['id']}/scores",
        headers=headers,
    )
